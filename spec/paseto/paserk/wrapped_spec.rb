# encoding: binary
# typed: false
# frozen_string_literal: true

RSpec.describe Paseto::Paserk::Wrapped do
  let(:wrapping_key) { Paseto::Util.decode_hex('707172737475767778797a7b7c7d7e7f808182838485868788898a8b8c8d8e8f') }
  let(:unwrapped) { Paseto::Util.decode_hex('')}
  let(:paserk) { 'hLBw_r4wY1mTy9hu27ibzOnFfW37jc3SER1fu3x2sh0cyE31abqEFzocZRg-KgAGY7syAoMsBZZi7ZlJ4xFreLdCi7cQWpIH3ejUQ5WIRMRDK2lh4X9knh2Bj26XaBOSqtK62ACF-V2utRiI3QVeOOLhH-GWuD8RovVQvA1Vv5Q' }

  it 'is good' do
    expect(described_class.unwrap('k3', 'secret-wrap', wrapping_key, paserk).public_to_pem).to eq(unwrapped)
  end
end
